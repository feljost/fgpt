...
def run_experiment(
    ...
    cfg = FGPTConfig(gradient_checkpointing=True, rope_base=rope_base, n_head=n_head, diff_attn=diff_attn)
    model = FGPT(cfg)
    model.to("cuda")
...
def main():
    ...
    parser.add_argument("--diff-attn", action="store_true", default=False)
